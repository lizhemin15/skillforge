...
装完打开最后一行打印的「访问地址」就能用了。默认它是安装时探测到的本机内网 IP
（`http://<内网IP>:8092`）；如果这台机器走反向代理或有多张网卡，改用
`--public-url https://你的域名` 重装，或装完改 `skillforge.env` 里的
`SKILLFORGE_PUBLIC_URL` 再重启服务 —— 这个值只影响链接里写的主机名，
但填错了表现是「同事打得开页面、下载却失败」，很容易误判成服务坏了。

只有「就在本机浏览器里用」的场景才适合 `http://localhost:8092`。
