...
MUTATIONS = [
    ...
    (
        # 锚点跟着实现走（2026-09-22 漂过一次）：材料渲染从那串内联三元表达式里被抽成了
        # `matOf(s)`（日志加进来之后，渲染分支从 2 条变 3 条，内联写法没法读也没法测），
        # 老锚点整段消失 → 脚本按规矩判「注入点没找到」→ preflight 红。
        # 那次红是对的（同 M4，锚点漂了等于这把尺子不再量任何东西），但修法有讲究：
        # **锚点改挂在调用点**（`matOf(s) +`），不再挂渲染表达式 —— 渲染实现还会继续长
        # （日志/旁白/进度条），调用点这一行才是「材料到底渲不渲」的稳定边界。
        # 注入体仍是最小的「只改被测行为」：把渲染结果换成空串，其余一律不动。
