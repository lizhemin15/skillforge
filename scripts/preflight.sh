...
if [ "$QUICK" = 0 ]; then
  ...
  # 「本轮用户原话必须真的进执笔跳 prompt」的变异自证（2026-09-22）。守的是用户投诉②
  # 「生成的 skill 跟我给的内容完全没关系」：根因是 chat.go 在 Push 之前取 history、
  # 写作链路签名里没有 userMsg，于是模型只能照抄范文里的 {公司名称}/{日期} 或自编数字。
  # 五条变异：拆执笔跳注入 / 拆构思跳注入 / 去掉占位符禁令 / 调换块顺序 / 空消息硬塞块。
  # 脚本自己踩过一次**空跑绿**（`-run "'A|B|C'"` 的单引号被 Go 当字面正则，3 条用例
  # 只有 1 条真跑），所以它多了一条判据：基线必须逐条看到预期用例的 `-- PASS` 行。
  selfcheck '写作 / 原话进执笔跳尺子自证' python3 scripts/selftest_user_raw_ruler.py
  # 「提示词明说要正文，就不许被文档生成技能抢走」的四记变异自证（2026-09-23）。
  # 守的是线上取证过的一类交付形态翻转：同一份 134 字输入（末句「直接输出正文」）连跑 8 次，
  # 4 次命中 skill_type=docgen 的技能 → 屏幕上只有文件卡片和 39 字回执，正文一个字没有。
  # 四记变异分别打：闸门永不弃技能 / 去掉 manual 例外 / 不看技能类型（反向故障）/ 拿题材词当判据。
  # 挂在这里而不是只进 Go 单测的原因：它要**临时改写源码**再跑，与 go test 那步必须串行。
  selfcheck '写作 / 交付形态闸门变异自证' bash scripts/mutate_textonly_route.sh
  ...
  # 零材料强制门：一句话指令（14 字）+ 零材料时，分类器一条 needs 都不报，旧实现里
