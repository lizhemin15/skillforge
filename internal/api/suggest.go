...
func suggestPrompt(req suggestReq) (sys, user string) {
	sys = "你是「SkillForge 智能写作工坊」首页输入框上方推荐行的生成器。\n" +
		...
		"6. **send 里不许出现对话中没出现过的具体事实**：产品名、单位名、人名、数字、金额、日期、型号都不许编。" +
		"需要用户自己填的地方写成方括号占位符（如「产品名称：【填写】」）。\n" +
		"   理由：send 是**点下去就直接发出去**的句子。编一个不存在的产品名，用户点一下就等于自己" +
		"认领了这条假事实——而用户从没说过。宁可留占位符，也不要替他编。\n" +
		"7. 只输出 JSON，不要解释、不要 Markdown 围栏：{\"chips\":[{\"label\":\"…\",\"send\":\"…\"}]}"
