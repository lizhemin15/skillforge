...
func (e *Engine) generateSys(sc *SkillContent) string {
	...
	if strings.TrimSpace(sc.Template) != "" {
		sys += "\n\n==== 回答骨架模板 ====\n" + sc.Template
		// 骨架模板里带 {xxx} 占位符（{公司名称}/{日期}/{关键数据}…）。这条硬规则必须挂在
		// **模板本身旁边**，而不是只挂在「本轮用户原话」那一块上：userRawBlock 在没有用户文本时
		// 会整体消失，规则跟着一起消失，占位符就又漏进正文了。
		// 线上账本（2026-09-22 实测，vague 腿）：我方只说「帮我写个新闻稿。」，
		// 正文前 120 字 = 「**{公司名称}完成{关键数据}{事件性质}**{日期}，{公司名称}宣布…」，
		// 整篇 36 个占位符 —— 用户的原话就是「生成的 skill 和我给的内容完全没有关系」。
		sys += "\n\n==== 正文书写规则（硬约束）====\n" +
			"1) 上面骨架模板/范文里的 {xxx} 是**占位符标记，不是稿子**，正文里的每一处都必须换成真实内容；" +
			"把 {公司名称} 这种 token 原样留在正文里视为交付失败。\n" +
			"2) 确实没有依据的**个别**字段：写成中文占位 【待补:字段名】（例：【待补:签约客户数】），" +
			"一处占位只顶替那一处，不要因为某个字段缺失就整篇改用占位符。\n" +
			"3) 连主体都定不下来（公司/机构名称、日期、核心事件一个都没有依据）时：**不要交稿**，" +
			"直接问用户这几项是什么，最多给 3 个问题，每条都引用用户这次的原话；" +
			"交一份满是 {xxx} 的稿子等同于没干活。"
	}
